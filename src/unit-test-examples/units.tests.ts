...
describe('concat', () => {
  ...
  it('should concat well and being', () => {
    const result = concat('well', 'being');
    expect(result).to.equal('wellbeing');
  });

  //Have to complete this
  /* it('should throw an error string is missing', () => {
    expect(()=>{ concat('work', null) }).to.throw('String is empty')
  }); */
